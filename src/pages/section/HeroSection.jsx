...
// 1. Move static data outside to prevent recreation on re-render
const SLIDES = [
  {
    title: "Core Stack",
...
// 2. Memoize Typewriter to prevent the whole Hero from re-rendering during typing
...
const DelinPremiumHero = ({ id }) => {
  ...
  // 3. Transform values with CSS hardware acceleration hints
  ...
  useEffect(() => {
    ...
    }, 5000); // Increased to 5s to allow for full typing/reading
  ...
  return (
    <section id={id} className="relative min-h-screen bg-[#050505] text-white selection:bg-[#B9FF66] overflow-hidden font-sans">
      ...
      {/* 4. Optimized Background (using CSS for the grain) */}
      <div className="absolute inset-0 z-0 opacity-10 pointer-events-none will-change-auto" 
      ...
      <div className="relative z-10 w-full max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-12 xl:px-16 py-20 md:py-32">
        <div className="flex flex-col lg:grid lg:grid-cols-12 gap-16 items-start">
          ...
          <div className="w-full lg:col-span-7">
            ...
            <div className="min-h-[260px] md:min-h-[380px]">
              <AnimatePresence mode="wait">
                <motion.h1
                  ...
                  className="text-5xl sm:text-7xl md:text-8xl font-bold tracking-tight leading-[0.9] mb-10 will-change-transform"
                >
            ...
            {/* Actions */}
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }} className="flex flex-wrap gap-6 items-center">
...
<div className="w-full lg:col-span-5 relative group">
  {/* 1. The outer glow/border gradient (High-end UI trick) */}
  ...
  <div className="relative bg-[#0A0A0A]/90 backdrop-blur-3xl border border-white/10 p-8 md:p-10 rounded-[40px] overflow-hidden min-h-[500px] flex flex-col justify-between">
        ...
        <div className="space-y-3">
          {SLIDES[currentSlide].items.map((item, i) => (
            <motion.div
              ...
              className="flex items-center justify-between p-5 rounded-[24px] bg-white/[0.03] border border-white/[0.05] hover:bg-white/[0.07] hover:border-white/10 transition-all duration-300 group/item"
            >
              <div className="flex items-center gap-4">
                <div className="p-2 rounded-lg bg-black/40 text-slate-400 group-hover/item:text-[#B9FF66] transition-colors">
                  {item.icon}
                </div>
                <span className="text-sm font-medium text-slate-300 tracking-wide">{item.label}</span>
              </div>
              <span className="text-sm font-mono text-[#B9FF66] font-semibold tracking-tighter">
                {item.val}
              </span>
            </motion.div>
          ))}
    ...
    <div className="mt-8 pt-8 border-t border-white/5 flex items-center justify-between text-[10px] font-mono text-slate-500 tracking-[0.2em]">
       <div className="flex items-center gap-4">
          <span className="hover:text-white transition-colors cursor-default">EST. 2024</span>
          <span className="w-[1px] h-3 bg-white/10"></span>
          <span className="hover:text-white transition-colors cursor-default">SYSTEM_v4</span>
       </div>
       <div className="flex gap-1.5">
         {SLIDES.map((_, idx) => (
           <div 
            key={idx}
            className={`h-1 rounded-full transition-all duration-700 ${idx === currentSlide ? 'w-8 bg-[#B9FF66]' : 'w-2 bg-white/10'}`} 
           />
         ))}
       </div>
    </div>
